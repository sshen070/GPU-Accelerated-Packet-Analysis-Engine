...
int main(int argc, char* argv[]) {
    ...
    // Create buffers for portscan
    GPUPortScanState portscan_state[2];

    init_portscan(portscan_state[0], BATCH_SIZE);
    init_portscan(portscan_state[1], BATCH_SIZE);
    ...
    while (!eof){
        ...
        GPUPortScanState& ps = portscan_state[batch_id % 2];
        ...
        launch_portscan(ps, batch.d_batch.src_ip, batch.d_batch.dst_port, batch.d_batch.count);

        // Track top 4 IPs & counts per batch
        uint32_t top_hits[4];
        uint32_t counts[4];

        collect_portscan(ps, 100, top_hits, counts);
        ...
        for (uint32_t i = 0; i < 4; i++) {
            ...
            struct in_addr addr;
            addr.s_addr = top_hits[i];

            std::cout << inet_ntoa(addr) << " -> " << counts[i] << " connections\n";
        }

        batch_id++;
    }

    std::cout << "All Batches Ran...\n\n";
    ...
    // Free GPU filering buffers
    destroy_pipeline(pipeline_slot[0]);
    destroy_pipeline(pipeline_slot[1]);

    // Free GPU portscan buffers 
    destroy_portscan(portscan_state[0]);
    destroy_portscan(portscan_state[1]);
